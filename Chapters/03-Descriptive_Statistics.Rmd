...
coffee <- c(0, 0, 0, 1, 1, 1, 1, 2, 2, 4, 7)
...
- For skewness, look at where most values are concentrated and which side has the longer tail.
</div>
</details>
...
**(a)** **Right-skewed** — most students drink 0–2 cups, while a few larger values, especially 4 and 7 cups, create a long tail to the right.

**(b)**
$$\bar{x} = \frac{0+0+0+1+1+1+1+2+2+4+7}{11} = \frac{19}{11} \approx 1.73 \text{ cups}$$
Median = 6th value = **1 cup**

**(c)** Mode = **1** (appears 4 times, the tallest stack)

**(d)** Replacing 7 with 12 gives a new total of 24, so the new $\bar{x} = \frac{24}{11} \approx 2.18$ (increased by ~0.45); median remains **1**. The **mean is sensitive to outliers**; the median is **robust**.
</div>
</details>
