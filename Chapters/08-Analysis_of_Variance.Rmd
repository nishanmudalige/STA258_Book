...
ggplot(df_forms, aes(x = form, y = time, fill = form)) +
  ...
  labs(x = "Tax Return Form", y = "Completion time (minutes)",
       caption = "n = 10 per form  |  ANOVA: F = 14.90, p-value = 1.82e-06") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "none",
        plot.caption = element_text(color = "#555", size = 9))
```

A one-way ANOVA gives $F^* = 14.90$, p-value $= 1.82 \times 10^{-6}$. Which conclusion is best at $\alpha = 0.05$?
...
<summary>💡 Hint</summary>
<div class="content">
- Compare p-value $= 1.82 \times 10^{-6}$ to $\alpha = 0.05$.
...
Since p-value $= 1.82 \times 10^{-6} \ll \alpha = 0.05$, we reject $H_0$. The boxplot shows Form 3 has a visibly higher median and wider spread, which is consistent with at least one group differing. However, ANOVA alone establishes only that **at least two population means differ** — it does not identify which specific pair(s). Post-hoc pairwise comparisons (e.g., Fisher's LSD, Bonferroni) are needed to determine which forms differ.
</div>
</details>
...
```{r ch8-p4-box, echo=FALSE, fig.height=3.8, fig.width=7, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
