...
**Calculation:**
$$n = \frac{(2.576)^2(0.18)(0.82)}{(0.02)^2} = \frac{6.6358 \times 0.1476}{0.0004} = \frac{0.9795}{0.0004} = 2448.75$$
...
A researcher records the exam scores of $n = 25$ students. The histogram below shows the distribution of scores, with the dashed line marking the sample mean.

```{r ch5-q40-hist, echo=FALSE, fig.height=3.5, fig.width=6, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
ggplot(df_scores, aes(x = score)) +
  geom_histogram(binwidth = 5, fill = "#619CFF", color = "white", boundary = 60) +
  geom_vline(xintercept = mean(scores), linetype = "dashed", color = "#e17055", linewidth = 0.9) +
  annotate("text", x = mean(scores) + 1, y = 6.4, label = paste0("bar(x) == ", round(mean(scores),1)),
           parse = TRUE, color = "#e17055", size = 4, hjust = 0) +
  scale_x_continuous(breaks = seq(60, 100, 5)) +
  labs(x = "Exam Score", y = "Frequency",
       caption = "n = 25 students  |  Dashed line = sample mean") +
  theme_minimal(base_size = 12) +
  theme(plot.caption = element_text(color = "#555", size = 9))
```

**(a)** Describe the shape of the distribution shown in the histogram.
...
<summary>💡 Hint</summary>
<div class="content">
- The $t$-interval formula is $\bar{x} \pm t^*_{n-1} \cdot s/\sqrt{n}$.
...
<summary>✅ Answer</summary>
<div class="content">
**(a)** The histogram is roughly symmetric and bell-shaped, centred around 78–79 with no strong skew or outliers.
...
```{r ch5-q41-box, echo=FALSE, fig.height=3.5, fig.width=5, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
**(b)** Construct a **95% Welch $t$-CI** for $\mu_X - \mu_Y$. Do not assume equal variances.
</div>
...
```{r ch5-q42-hist, echo=FALSE, fig.height=3.5, fig.width=6, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r ch5-q43-box, echo=FALSE, fig.height=3.5, fig.width=5, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
Based on the boxplots, the two groups appear to have different means. Using the dataset `plant_yield.csv`, construct a **90% Welch $t$-CI** for $\mu_A - \mu_B$. Do not assume equal variances.
