...
Complete the R code below to (a) compute the exact multinomial probability of observing *this specific* count vector under $H_0$ using `dmultinom()`, (b) find the expected counts, (c) compute the chi-square statistic, and (d) find the degrees of freedom and p-value.

<div class="webr-exercise">
<div class="webr-exercise-header">Exercise</div>
<textarea class="webr-editor" rows="20">observed <- c(90, 40, 20, 10)
probs    <- c(9/16, 3/16, 3/16, 1/16)
n        <- sum(observed)

# (a) Exact multinomial probability of this precise count vector under H0
dmultinom(___, prob = ___)

# (b) Expected counts under H0
expected <- n * probs
expected
...
<summary>💡 Hint</summary>
<div class="content">
- `dmultinom(x, prob)` returns the exact joint multinomial probability of observing *exactly* the count vector `x`, given the category probabilities `prob` — this is a single, very small number, not a p-value.
- The chi-square goodness-of-fit p-value, by contrast, is the probability the *statistic* is at least as large as the value you observed, computed from the chi-square distribution — the second blank should be the statistic you just calculated in part (c).
- Don't confuse the two probabilities: they answer different questions.
</div>
</details>
...
**Interpretation:** The `dmultinom()` result ($\approx 0.0000312$) is simply the probability of this *one exact* count pattern among the enormous number of possible patterns — it is not, by itself, evidence against $H_0$. The relevant test uses $\chi^2 \approx 6.667$ with $df = 3$, giving a p-value $\approx 0.0833$. Since this p-value exceeds $\alpha = 0.05$, we fail to reject $H_0$: the observed offspring counts are reasonably consistent with the classical 9:3:3:1 ratio, even though the second phenotype (observed 40 vs. expected 30) shows some departure from what was hypothesized.
</div>
</details>
