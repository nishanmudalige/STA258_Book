...
```{r fig.cap="An illustration of simple linear regression model (with positive correlation between total sleep duration and memory score).", echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
...
```{r fig.cap="An illustration of strong positive correlation ($r_{xy} \\approx +1$).", message=FALSE, warning=FALSE, echo=FALSE, fig.align='center'}
library(ggplot2)

set.seed(123)  # for reproducibility

# Data for blue points (on y ≈ x with noise)
blue_df <- data.frame(
  x = seq(0.5, 5, by = 0.5),
  y = seq(0.5, 5, by = 0.5) + rnorm(10, mean = 0, sd = 0.1)  # add noise here
)

# Data for blue points (on y ≈ x + 0.5 with noise)
red_df <- data.frame(
  x = seq(0.5, 5, by = 0.5),
  y = seq(1, 5.5, by = 0.5) + rnorm(10, mean = 0, sd = 0.1)  # and here
)

# Plot
ggplot() +
  geom_point(data = blue_df, aes(x = x, y = y), color = "#619CFF", size = 2) +
  geom_point(data = red_df, aes(x = x, y = y), color = "#619CFF", size = 2) +
...
```{r fig.cap="An illustration of strong negative correlation ($r_{xy} \\approx -1$).", message=FALSE, warning=FALSE, echo=FALSE, fig.align='center'}
library(ggplot2)

set.seed(456)  # ensures reproducibility

# Blue points: y decreases with x + noise
x_vals <- c(0.5, 1, 1.5, 2.3, 2.5, 3.2, 3.5, 4.1, 4.5, 5)
blue_df <- data.frame(
  x = x_vals,
  y = rev(x_vals) + rnorm(10, mean = 0, sd = 0.1)  # reversed x with noise
)

# Red points: slightly off the main trend with noise
red_df <- data.frame(
  x = x_vals,
  y = rev(x_vals) + 0.2 + rnorm(10, mean = 0, sd = 0.1)  # slight offset + noise
)
...
```{r fig.cap="An illustration of no correlation ($r_{xy} \\approx 0$).", message=FALSE, warning=FALSE, echo=FALSE, fig.align='center'}
library(ggplot2)

set.seed(789)

x_vals <- c(0.5, 1, 1.5, 2.3, 3.1, 4.0, 4.5, 5.2, 6.0)

df <- data.frame(
  x = x_vals,
  y = runif(length(x_vals), min = 0.5, max = 5),  # random y
...
ggplot(df, aes(x = x, y = y)) +
  ...
  scale_color_manual(values = c("blue" = "#619CFF", "red" = "#619CFF")) +
  labs(x = expression(x), y = expression(y)) +
  theme_minimal() +
