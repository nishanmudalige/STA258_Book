...
```{r fig.slr, fig.align='center', fig.cap="An illustration of simple linear regression. The blue points are measures of height monthly, and the red line is our SLR model. In this case $m$ is the slope, $b$ is the intercept.", echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)

# Data
points_df <- data.frame(
  Time = c(0.5, 1.5, 1, 2, 2.5, 3, 3.5, 4, 4.5, 5),
  Height = c(0.75, 1.75, 1.25, 2.1, 2.4, 2.8, 3.1, 3.7, 3.95, 4.1)
)

line_df <- data.frame(Time = seq(0, 6, length.out = 100))
line_df$Height <- 0.8 * line_df$Time + 0.35

# Plot
ggplot() +
  geom_point(data = points_df, aes(x = Time, y = Height), color = "#619CFF", size = 2) +
  geom_line(data = line_df, aes(x = Time, y = Height), color = "#F8766D", size = 1) +
  annotate("text", x = 5.5, y = 4.5, label = "y = mx + b", hjust = 0, color = "#F8766D") +
  labs(x = "Time (month)", y = "Height (m)") +
  theme_minimal()
```
...
```{r fig.correlation, echo=FALSE, fig.align='center', fig.cap="An illustration of strong positive correlation ($r_{xy} \\approx +1$).", message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r fig.negative, echo=FALSE, fig.align='center', fig.cap="An illustration of strong negative correlation ($r_{xy} \\approx -1$).", message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r fig.nocorrelation, echo=FALSE, fig.align='center', fig.cap="An illustration of no correlation ($r_{xy} \\approx 0$).", message=FALSE, warning=FALSE}
library(ggplot2)
