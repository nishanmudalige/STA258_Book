...
Researchers find out that longer sleep plays crucial role in memory consolidation. It helps people to save and solidify new information, making it less likely to be forgotten.

```{r fig.align='center', fig.cap="An illustration of simple linear regression model (with positive correlation between total sleep duration and memory score).", echo=FALSE, message=FALSE, warning=FALSE}
...
fig <- plot_ly(data, x = ~sleep, y = ~memory, type = 'scatter', mode = 'markers',
  ...
  add_trace(y = ~fit, mode = 'lines',
            ...
            name = 'SLR Line') %>%
  layout(title = "Simple Linear Regression: Total Sleep Duration vs Memory Score",
         xaxis = list(title = "Hours of Sleep"),
         yaxis = list(title = "Memory Score"),
...
From figure 9.1 above, you have probably already noticed that as total sleep duration increases (increase in x-axis or independent variable), the memory score also increases (increase in y-axis or dependent variable). If this holds, then we say that there exists a positive correlation between total sleep duration (independent variable) and memory score (dependent variable).

**Example of Negative Correlation**

Now, let's consider another scenario that you want to drive from Toronto to Vancouver. You are going to arrive in Vancouver with about 42.06 hours, if you drive your car with average speed 100 km/h. Trivially, you will arrive in Vancouver faster (35.05 hours), if you drive your car with average speed 120 km/h.

```{r fig.align='center', fig.cap="An illustration of simple linear regression model (with negative correlation between car speed and total travel time).", echo=FALSE, message=FALSE, warning=FALSE}
...
fig <- plot_ly(data, x = ~speed, y = ~travel_time, type = 'scatter', mode = 'markers',
               marker = list(color = 'rgba(255, 120, 0, 0.7)', size = 8),
               name = 'Data points') %>%
  add_trace(y = ~fit, mode = 'lines',
            line = list(color = 'blue', width = 2),
            name = 'SLR Line') %>%
  layout(title = "SLR: Car Speed vs Travel Time",
         xaxis = list(title = "Average Speed (km/h)"),
         yaxis = list(title = "Travel Time (hours)"),
         legend = list(x = 0.05, y = 0.95))
...
From figure 9.2 above, you have probably already noticed that as average car speed increases (increase in x-axis or independent variable), the total travel time decreases (decrease in y-axis or dependent variable). If this holds, then we say that there exists a negative correlation between average car speed (independent variable) and total travel time (dependent variable).

**Example of No Correlation**

Finally, let's consider the relationship between coffee consumption and intelligence. There is no evidence to prove that someone drink coffee will lead to increase or decrease in intelligence.

```{r fig.align='center', fig.cap="An illustration of simple linear regression model (with no correlation between coffee consumption and intelligence).", echo=FALSE, message=FALSE, warning=FALSE}
# Load library
library(plotly)
...
fig <- plot_ly(data, x = ~coffee, y = ~intelligence, type = 'scatter', mode = 'markers',
               marker = list(color = 'rgba(0, 100, 255, 0.7)', size = 8),
               name = 'Data points') %>%
  layout(title = "Coffee Consumption vs Intelligence (No Correlation)",
         xaxis = list(title = "Coffee Consumption (cups/day)"),
...
In this case, if we cannot conclude that an increase in coffee consumption (independent variable) will lead to either an increase or a decrease in intelligence (dependent variable), then we say there is no correlation between coffee consumption (independent variable) and intelligence (dependent variable).

Also, we are going to discuss more about correlation in the following subsection.
...
- If $r_{xy} \approx +1$, then we say that the two random variables have
  a strong positive correlation. (See figure 9.2)
  

```{r echo=FALSE, fig.align='center', fig.cap="An illustration of strong positive correlation ($r_{xy} \\approx +1$).", message=FALSE, warning=FALSE}
library(ggplot2)

# Data for blue points (on y = x)
blue_df <- data.frame(
  x = seq(0.5, 5, by = 0.5),
  y = seq(0.5, 5, by = 0.5)
)

# Data for red points (also 11 points)
red_df <- data.frame(
  x = seq(0.5, 5.5, by = 0.5),
  y = seq(1, 6, by = 0.5)  # now also 11 values
)

# Plot
ggplot() +
  geom_point(data = blue_df, aes(x = x, y = y), color = "#619CFF", size = 2) +
  geom_point(data = red_df, aes(x = x, y = y), color = "#F8766D", size = 2) +
...
- If $r_{xy} \approx -1$, then we say that the two random variables have
  a strong negative correlation. (See figure 9.3)


```{r echo=FALSE, fig.align='center', fig.cap="An illustration of strong negative correlation ($r_{xy} \\approx -1$).", message=FALSE, warning=FALSE}
library(ggplot2)

# Blue points (strong negative correlation)
blue_df <- data.frame(
  x = c(0.5, 1, 1.5, 2.3, 2.5, 3.2, 3.5, 4.1, 4.5, 5),
  y = c(5, 4.5, 4, 3.5, 3, 2.5, 2, 1.5, 1, 0.5)
)

# Red points (slightly off diagonal)
red_df <- data.frame(
  x = c(0.5, 1, 1.5, 2.3, 2.5, 3.2, 3.5, 4.1, 4.5, 5),
  y = c(4.5, 4, 3.5, 3, 2.5, 2, 1.5, 1, 0.5, 0)
)
...
```{r echo=FALSE, fig.align='center', fig.cap="An illustration of no correlation ($r_{xy} \\approx 0$).", message=FALSE, warning=FALSE}
library(ggplot2)

# Points from TikZ
df <- data.frame(
  x = c(0.5, 1, 1.5, 2.3, 3.1, 4.0, 4.5, 5.2, 6.0),
  y = c(2.7, 1.5, 4.1, 2.0, 0.8, 3.5, 1.2, 4.3, 2.5),
  color = c("blue", "red", "blue", "red", "blue", "red", "blue", "red", "blue")
)

# Plot
ggplot(df, aes(x = x, y = y)) +
  geom_point(aes(color = color), size = 2) +
  geom_hline(yintercept = 1.3, color = "black", linewidth = 1) +
  scale_color_manual(values = c("blue" = "#619CFF", "red" = "#F8766D")) +
  labs(x = expression(x), y = expression(y)) +
  theme_minimal() +
  theme(legend.position = "none")
```

We are going to provide an interactive correlation figure (see the one below). The aim of the figure is to give you an illustration about where data locates as correlation changes.

```{r CorrShinyApp, echo=FALSE, warning=FALSE, results='asis'}
cat('<div style="border: none;">')
knitr::include_app("https://bryan-su.shinyapps.io/desktop/", height = "743")
cat('</div>')
```
...
```{r, echo=FALSE, out.width='90%',fig.align='center',fig.cap= "An illustration of Total Sum of Squares (SST). The blue points represent height measurements over time, the dashed line is the mean height $\\bar{y}$, and the solid black vertical lines represent the squared deviations from the mean (SST components)."}
library(ggplot2)
...
```{r, echo=FALSE, fig.align='center', out.width='90%', fig.cap= "A simple linear regression illustration with residuals shown. Blue points are observed data, red line is the model, dashed line is the average of $y$, and black lines represent residuals $y_i - \\hat{y}_i$. "}
library(ggplot2)
...
```{r, echo=FALSE, out.width='90%',fig.align='center', message=FALSE, warning=FALSE, results='hide', fig.cap = "Visual representation of regression variability. The blue point represents the observed value $y_i$, the red point shows the predicted value $\\hat{y}_i$, and the dashed line represents the mean $\\bar{y}$. The green arrow illustrates the total variability (SST), the pink arrow shows the explained variability (SSR), and the orange curly bracket represents the residual (unexplained) variability (SSE)."}
library(ggplot2)
library(grid)
...
# Full ggplot
ggplot(df, aes(x = x, y = y_i)) +
  # Axes
  geom_segment(aes(x = -1, xend = 7, y = 0, yend = 0), 
               arrow = arrow(length = unit(0.2, "cm")), color = "#000000") +
  geom_segment(aes(x = 0, xend = 0, y = -1, yend = 5), 
               arrow = arrow(length = unit(0.2, "cm")), color = "#000000") +

  # Observed y_i
  geom_point(aes(x = x, y = y_i), color = "#619CFF", size = 3) +
  annotate("text", x = df$x, y = df$y_i + 0.3, label = "Observed~y[i]", parse = TRUE, hjust = 0) +

  # Predicted yhat_i
  geom_point(aes(x = xhat, y = yhat_i), color = "#F8766D", size = 3) +
  annotate("text", x = df$xhat + 0.3, y = df$yhat_i - 0.4, label = "hat(y)[i]", parse = TRUE, hjust = 0) +

  # Regression line
  stat_function(fun = function(x) 0.8 * x + 0.35, color = "#999999", size = 1) +
  annotate("text", x = 6.2, y = 5.1, label = "hat(y)==hat(beta)[0] + hat(beta)[1]*x", parse = TRUE, hjust = 0) +

  # Mean line
  geom_segment(aes(x = -1, xend = 7, y = y_bar, yend = y_bar), 
               linetype = "dashed", color = "black") +
  annotate("text", x = 7, y = df$y_bar + 0.1, label = "bar(y)", parse = TRUE, hjust = 1) +

  # SST
  geom_segment(aes(x = x, xend = x, y = y_i, yend = y_bar), 
               color = "#00BA38", size = 1,
               arrow = arrow(type = "closed", length = unit(0.15, "cm"))) +
  annotate("text", x = df$x + 0.3, y = (df$y_i + df$y_bar)/2 + 0.15, 
           label = "SST", size = 3, color = "#00BA38") +

  # SSR
  geom_segment(aes(x = xhat, xend = xhat, y = yhat_i, yend = y_bar), 
               color = "#FF62BC", size = 1,
               arrow = arrow(type = "closed", length = unit(0.15, "cm"))) +
  annotate("text", x = df$xhat - 0.3, y = (df$yhat_i + df$y_bar)/2 - 0.15, 
           label = "SSR", size = 3, color = "#FF62BC") +

  # SSE
  geom_curve(
    aes(x = xhat - 0.1, y = yhat_i, xend = xhat - 0.1, yend = y_i), 
    color = "#ED8141", curvature = -0.5,
    arrow = arrow(type = "closed", length = unit(0.15, "cm"))
  ) +
  annotate("text", x = df$xhat - 0.6, y = (df$y_i + df$yhat_i)/2 - 0.05, 
           label = "SSE", size = 3, color = "#ED8141") +
