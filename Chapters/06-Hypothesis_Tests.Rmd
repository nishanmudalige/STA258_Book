...
A university study reports that the standard deviation of weekly study hours among undergraduates is $\sigma_0 = 4$ hours. A researcher surveys a random sample of $n = 25$ students in an engineering program and finds a sample standard deviation of $s = 6$ hours. Test whether the variance of study hours for engineering students differs from the university-wide value. Use $\alpha = 0.05$. Assume study hours are approximately normally distributed.
...
- Since $s > \sigma_0$, the test statistic will be in the upper tail. The two-tailed p-value is $2 \times P(\chi^2_{n-1} > \chi^{2*})$.
- Note that $\sigma_0^2 = 4^2 = 16$.
</div>
</details>
...
**(b)**
$$\chi^{2*} = \frac{(n-1)s^2}{\sigma_0^2} = \frac{(25 - 1)(6^2)}{4^2} = \frac{(24)(36)}{16} = \frac{864}{16} = 54$$

**(c)** Degrees of freedom: $df = n - 1 = 24$.

Since $s > \sigma_0$, the statistic lies in the upper tail. For $H_a: \sigma^2 \ne 16$ (two-tailed):
$$\text{p-value} = 2 \times P(\chi^2_{24} > 54) \approx 2(0.0003) = 0.0006$$
<a href="foundations-of-inference.html#distribution-calculator" target="_blank" style="font-size:0.85em; color:#2c7bb6;">🔢 Distribution Calculator</a>

**(d)** Since p-value $\approx 0.0006 < \alpha = 0.05$, we **reject $H_0$**. There is sufficient evidence at the 5% level to conclude that the variance of weekly study hours for engineering students differs from the university-wide value of $\sigma_0^2 = 16$ hr$^2$.
</div>
</details>
...
A college reports that 111 of 150 students in its online section ($n_1 = 150$) and 96 of 120 students in its in-person section ($n_2 = 120$) completed the course. Is the online completion rate significantly lower than the in-person rate?
...
**(b)** $\hat{p}_1 = 111/150 = 0.740$, $\hat{p}_2 = 96/120 = 0.800$.

$$\hat{p} = \frac{111+96}{150+120} = \frac{207}{270} \approx 0.7667$$

$$z^* = \frac{0.740-0.800}{\sqrt{0.7667 \times 0.2333 \times (1/150+1/120)}} = \frac{-0.060}{\sqrt{0.1789 \times 0.01500}} = \frac{-0.060}{\sqrt{0.002683}} = \frac{-0.060}{0.05180} \approx -1.159$$

**(c)** For $H_a: p_1 < p_2$ (lower-tailed):
$$\text{p-value} = P(Z < -1.159) \approx 0.123$$
<a href="foundations-of-inference.html#distribution-calculator" target="_blank" style="font-size:0.85em; color:#2c7bb6;">🔢 Distribution Calculator</a>

**(d)** Since p-value $\approx 0.123 > \alpha = 0.05$, we **fail to reject $H_0$**. There is insufficient evidence to conclude that the online section has a lower completion rate than the in-person section.
</div>
</details>
...
Two quality inspectors each rate the same set of parts on a scale from 1 to 10. Inspector A rates $n_1 = 20$ parts ($s_A = 2.3$ points); Inspector B rates $n_2 = 18$ parts ($s_B = 1.9$ points). Is there a significant difference in the variability of their ratings?

You may use the Distribution Calculator in Section 4.1.5 to find the required tail probability.

(a) State $H_0$ and $H_a$.
(b) Compute the $F$ test statistic, placing the larger $s^2$ in the numerator.
(c) State the degrees of freedom and find the p-value.
...
- Placing the larger $s^2$ in the numerator keeps $F^* > 1$, making the upper-tail lookup straightforward.
- Two-tailed p-value $= 2 \times P(F_{df_1,df_2} > F^*)$.
- The critical value for a two-tailed $\alpha = 0.10$ test uses $\alpha/2 = 0.05$ in each tail.
</div>
</details>
...
**(b)** Since $s_A = 2.3 > s_B = 1.9$, place $s_A^2$ in the numerator:
$$F^* = \frac{s_A^2}{s_B^2} = \frac{2.3^2}{1.9^2} = \frac{5.29}{3.61} \approx 1.465$$

with $df_1 = 20-1 = 19$ and $df_2 = 18-1 = 17$.

**(c)** For $H_a: \sigma_A^2 \ne \sigma_B^2$ (two-tailed):
$$\text{p-value} = 2 \times P(F_{19,17} > 1.465) \approx 2(0.219) = 0.438$$
<a href="foundations-of-inference.html#distribution-calculator" target="_blank" style="font-size:0.85em; color:#2c7bb6;">🔢 Distribution Calculator</a>

The critical value $F_{0.05}(19,17) \approx 2.23$. Since $F^* = 1.465 < 2.23$, we confirm failure to reject.

**(d)** Since p-value $\approx 0.438 > \alpha = 0.10$, we **fail to reject $H_0$**. There is insufficient evidence to conclude that the two inspectors differ in the variability of their ratings.
</div>
</details>
...
Seven university students complete a self-reported anxiety questionnaire (scored 0–50; higher = more anxious) before and after a six-week mindfulness course. Their scores are:
...
<summary>💡 Hint</summary>
<div class="content">
- With $d_i = \text{After}_i - \text{Before}_i$, a decrease in anxiety gives $d_i < 0$.
...
**(b)** Differences $d_i = \text{After}_i - \text{Before}_i$: $\;{-8},\;{-12},\;{-5},\;{-9},\;{-3},\;{-11},\;{-7}$.
...
**(c)**
$$t^* = \frac{-7.857}{3.185/\sqrt{7}} = \frac{-7.857}{1.204} \approx -6.526, \qquad df = 6$$

**(d)** For $H_a: \mu_d < 0$ (lower-tailed):
