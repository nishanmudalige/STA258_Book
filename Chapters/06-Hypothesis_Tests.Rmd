...
```{r fig.cap="Two-tailed test: p-value is the total area in both tails beyond 卤test statistic", echo=FALSE, fig.align='center', out.width='70%'}
library(ggplot2)
...
- $P\text{�üvalue} \leq \alpha$-level, we reject $H_0$; We can conclude
...
- $P\text{�üvalue} > \alpha$-level, we fail to reject $H_0$; We cannot
...
satis铿ed? One simple method involves just looking at two sample
...
variances are reasonably close, you can be reasonably con铿dent that the
homogeneity assumption is satis铿ed and proceed with, for example,
...
- If samples are bigger than 10 % of the target population, we need to
  acknowledge this and note in our report. When we sample from a 铿nite
...
Can we conclude at the 5% signi铿cance level that the new fertilizer is
more e铿üective than the current one?
...
- The standard error under $H_0$ is $\sqrt{p_0(1-p_0)/n}$ �ü use $p_0$, not $\hat{p}$, here.
...
For each research scenario below, identify the **most appropriate hypothesis test** from the list, state the null and alternative hypotheses, and name the reference distribution.

Available tests:
- One-sample $z$ test for a mean ($\sigma$ known)
- One-sample $t$ test for a mean ($\sigma$ unknown)
- One-sample $z$ test for a proportion
- One-sample $\chi^2$ test for a variance
- Two-sample $z$ test for two means ($\sigma_1, \sigma_2$ known)
- Two-sample pooled $t$ test for two means (equal variances)
- Welch $t$ test for two means (unequal variances)
- Two-sample $z$ test for two proportions
- $F$-test for two variances
- Paired $t$ test

**(a)** A public health agency reports that the historical standard deviation of systolic blood pressure in adult males is $\sigma_0 = 14$ mmHg. A researcher measures 40 adult males and wants to determine whether today's population is **more variable** than the historical norm.

**(b)** An educator surveys a random sample of 180 first-year college students and asks whether they passed their high school calculus course. She wants to test whether the pass rate in her region exceeds the national figure of 68%.

**(c)** A pharmaceutical company conducts a crossover trial in which each of 12 patients receives both Drug A and Drug B in random order (one week apart). The company wants to determine whether Drug A produces a higher mean pain-relief score than Drug B.

**(d)** A factory runs two independent production lines. Line 1 uses equipment that has a known standard deviation of output of $\sigma_1 = 3.2$ kg, and Line 2 has a known standard deviation of $\sigma_2 = 2.8$ kg. Samples of sizes $n_1 = 50$ and $n_2 = 45$ are drawn. Management wants to test whether the two lines produce the same mean output.
</div>

<details data-type="hint">
<summary>💡 Hint</summary>
<div class="content">
- To choose among tests for a **mean**, first ask: is $\sigma$ known or unknown? Then ask: one sample or two? If two, are observations paired or independent?
- To choose between **pooled** and **Welch** $t$, ask whether the population variances can be assumed equal.
- For a **proportion**, the parameter is $p$ and no variance assumption is needed.
- For a **variance**, the parameter is $\sigma^2$ and the test statistic uses the $\chi^2$ distribution (one sample) or the $F$ distribution (two samples).
</div>
</details>
...
<summary>✅ Answer</summary>
<div class="content">
**(a) One-sample $\chi^2$ test for a variance**

The parameter of interest is the population variance $\sigma^2$. The researcher wants to test whether variability has **increased**:

$$H_0: \sigma^2 = 196 \quad \text{vs.} \quad H_a: \sigma^2 > 196$$

(where $\sigma_0^2 = 14^2 = 196$). Reference distribution: $\chi^2_{n-1} = \chi^2_{39}$ (right-tailed test).

---

**(b) One-sample $z$ test for a proportion**

The parameter of interest is the population proportion $p$. The researcher wants to test whether the regional pass rate **exceeds** 68%:

$$H_0: p = 0.68 \quad \text{vs.} \quad H_a: p > 0.68$$

Reference distribution: $Z \sim N(0,1)$ (upper-tailed test). The large-sample condition ($n\hat{p}$ and $n(1-\hat{p})$ both $\geq 10$) should be verified.

---

**(c) Paired $t$ test**

Each patient provides **two** observations (Drug A and Drug B), so observations are not independent. The relevant parameter is $\mu_d = \mu_A - \mu_B$:

$$H_0: \mu_d = 0 \quad \text{vs.} \quad H_a: \mu_d > 0$$

Reference distribution: $t_{n-1} = t_{11}$ (upper-tailed test).

---

**(d) Two-sample $z$ test for two means**

Both population standard deviations are **known** ($\sigma_1 = 3.2$, $\sigma_2 = 2.8$), so we use the $z$ statistic:

$$H_0: \mu_1 = \mu_2 \quad \text{vs.} \quad H_a: \mu_1 \ne \mu_2$$

Reference distribution: $Z \sim N(0,1)$ (two-tailed test).
</div>
</details>
...
$$\text{p-value} = P(\chi^2_{15} < 6.144) \approx 0.0228$$
<a href="foundations-of-inference.html#distribution-calculator" target="_blank" style="font-size:0.85em; color:#2c7bb6;">�� Distribution Calculator</a>
From the $\chi^2$-table: the lower 5th percentile of $\chi^2_{15}$ is $7.26$. Since $6.144 < 7.26$, the p-value $< 0.05$.

**(d)** Since p-value $\approx 0.023 < \alpha = 0.05$, we **reject $H_0$**. There is sufficient evidence at the 5% level to conclude that the variance of fill weights has decreased below $\sigma_0^2 = 25$ g$^2$ (i.e., $\sigma < 5$ g).
</div>
</details>
...
A researcher reads the following four study descriptions and wants to apply a one-sample $\chi^2$ test for a population variance in each case. For **each description**, assess whether the key assumption for the $\chi^2$ variance test is reasonably satisfied, and explain your reasoning.

The key assumption is: **the data come from a normally distributed population** (or the sample size is large enough for approximate normality of the chi-squared statistic, which for variance tests requires the underlying population to be normal — unlike the CLT, increasing $n$ alone does not rescue this test for non-normal data).

**(a)** A researcher measures the lengths of 200 randomly selected salmon in a river. He plots a histogram which shows a unimodal, roughly symmetric, bell-shaped distribution with no extreme outliers. He wants to test whether $\sigma^2 = 4$ cm$^2$.

**(b)** A hospital records the waiting times (minutes) of 15 patients in the emergency department. A stemplot of the data shows a strong right skew with one very large outlier (a patient who waited 6 hours while all others waited under 30 minutes). A researcher wants to test whether the variance of waiting times differs from a benchmark.

**(c)** A quality inspector measures the diameter (mm) of 12 precision ball bearings from a newly calibrated machine. Subject-matter knowledge indicates that machined component diameters from a well-controlled process are highly normally distributed. She wants to test $H_0: \sigma^2 = 0.01$.

**(d)** An economist records the annual income (\$) of 25 randomly chosen households in a city. A histogram shows a long right tail typical of income data. He wants to test whether $\sigma^2 = (15{,}000)^2$.
</div>

<details data-type="hint">
<summary>💡 Hint</summary>
<div class="content">
- The $\chi^2$ test for a population variance requires the **population** to be normally distributed.
- Unlike the $t$-test for a mean, this requirement is **not** relaxed by increasing $n$. Skewed or heavy-tailed populations make the chi-squared approximation unreliable even with large samples.
- Check: Is the histogram bell-shaped and symmetric? Are there extreme outliers? Does subject-matter knowledge support normality?
</div>
</details>
...
<summary>✅ Answer</summary>
<div class="content">
**(a) Assumption appears satisfied.**

A histogram showing a unimodal, symmetric, bell-shaped distribution with no extreme outliers is consistent with normality. With $n = 200$, we can also use a Q-Q plot or Shapiro–Wilk test to confirm. The $\chi^2$ variance test is appropriate here.

---

**(b) Assumption is seriously violated.**

Emergency-department waiting times with a strong right skew and a large outlier are far from normally distributed. A single extreme observation can dramatically inflate $s^2$, making the $\chi^2$ approximation unreliable. The test should **not** be applied without first addressing the outlier or using a non-parametric alternative.

---

**(c) Assumption appears satisfied.**

Diameters from a well-controlled machining process are typically normally distributed by design (small manufacturing tolerances arise from many independent small errors, invoking the CLT at the process level). With $n = 12$, the normality assumption is critical and is supported by domain knowledge. The $\chi^2$ test is appropriate.

---

**(d) Assumption is likely violated.**

Household income distributions are typically right-skewed (many moderate incomes, few very high incomes). A histogram showing a long right tail is inconsistent with normality. Despite the sample size of $n = 25$, the $\chi^2$ variance test is unreliable for skewed data. A logarithmic transformation of income may restore approximate normality before applying the test.
</div>
</details>
...
A researcher constructs a 95% confidence interval for a population mean $\mu$ and obtains $(12.4,\ 17.8)$. She is also interested in the two-sided hypothesis test:
$$H_0: \mu = \mu_0 \quad \text{vs.} \quad H_a: \mu \ne \mu_0$$
at $\alpha = 0.05$.

Use the duality between confidence intervals and two-sided hypothesis tests to answer the following questions **without computing any test statistic or p-value**.

(a) If $\mu_0 = 10$, should $H_0$ be rejected at $\alpha = 0.05$? Justify your answer.

(b) If $\mu_0 = 15$, should $H_0$ be rejected at $\alpha = 0.05$? Justify your answer.

(c) If $\mu_0 = 18$, should $H_0$ be rejected at $\alpha = 0.05$? Justify your answer.

(d) State the general rule that connects a 95% confidence interval to a two-sided test at $\alpha = 0.05$.
</div>

<details data-type="hint">
<summary>💡 Hint</summary>
<div class="content">
- Recall the duality result: **a level $\alpha$ two-sided test rejects $H_0: \mu = \mu_0$ if and only if $\mu_0$ falls outside a level $(1-\alpha)$ confidence interval for $\mu$**.
- Check whether $\mu_0$ is inside or outside the interval $(12.4,\ 17.8)$ for each part.
- The result is a two-tailed test at $\alpha = 0.05$, so the corresponding CI has confidence level $1 - 0.05 = 0.95$.
</div>
</details>
...
$\mu_0 = 10$ lies **outside** the 95% CI $(12.4,\ 17.8)$. By the duality principle, the two-sided test rejects $H_0: \mu = 10$ at the 5% level. Equivalently, the p-value of the test is less than 0.05.

---

**(b) Fail to reject $H_0$ at $\alpha = 0.05$.**

$\mu_0 = 15$ lies **inside** the 95% CI $(12.4,\ 17.8)$. By the duality principle, we fail to reject $H_0: \mu = 15$ at the 5% level. The p-value of the test exceeds 0.05.

---

**(c) Reject $H_0$ at $\alpha = 0.05$.**

$\mu_0 = 18$ lies **outside** the 95% CI $(12.4,\ 17.8)$ (since $18 > 17.8$). We reject $H_0: \mu = 18$ at the 5% level.

---

**(d) General rule:**

A level $\alpha$ **two-sided** significance test rejects $H_0: \mu = \mu_0$ at significance level $\alpha$ if and only if the value $\mu_0$ falls **outside** a $(1-\alpha) \times 100\%$ confidence interval for $\mu$. Equivalently, we fail to reject $H_0$ when $\mu_0$ lies inside the confidence interval.

This duality makes confidence intervals especially useful: a single $(1-\alpha)$ CI allows us to simultaneously test $H_0: \mu = \mu_0$ for **every possible** $\mu_0$ at level $\alpha$, without computing a separate test statistic or p-value each time.
</div>
</details>
...
- `t.test(x, y, alternative, var.equal)` �ü the first argument is the group you expect to have the larger mean when using `"greater"`.
</div>
</details>
...
A clinical researcher runs a paired study to evaluate whether a new cognitive training programme improves working memory scores. Each of the $n = 20$ participants is tested **before** and **after** an 8-week programme using a standardised working memory battery (scored 0–100). The researcher performs a paired $t$-test and obtains:

$$\bar{x}_d = 4.3 \text{ points},\quad s_d = 6.1 \text{ points},\quad t^* = 3.15,\quad df = 19,\quad \text{p-value} = 0.0054$$

where $d_i = \text{After}_i - \text{Before}_i$.

(a) State precisely what the p-value $0.0054$ means in the context of this study.

(b) The researcher concludes: *"The p-value of 0.0054 means there is only a 0.54% probability that the cognitive training programme has no effect."* Is this conclusion correct? Explain.

(c) At $\alpha = 0.05$, state the conclusion of the hypothesis test. Write your conclusion in the context of this study (do not just say "reject $H_0$").

(d) A colleague argues that a mean improvement of 4.3 points is too small to matter clinically (the minimum clinically important difference is 10 points). How does this observation illustrate the difference between **statistical significance** and **practical significance**?
</div>

<details data-type="hint">
<summary>💡 Hint</summary>
<div class="content">
- The p-value is a conditional probability: $P(\text{data at least as extreme as observed} \mid H_0 \text{ is true})$.
- The p-value is **not** the probability that $H_0$ is true, nor the probability that the result was due to chance.
- Statistical significance depends on $n$ as well as the size of the effect.
...
<summary>✅ Answer</summary>
<div class="content">
**(a)** The p-value of 0.0054 means: **assuming the cognitive training programme has no true effect on working memory** (i.e., $H_0: \mu_d = 0$ is true), the probability of observing a mean paired difference of 4.3 points or larger in a random sample of 20 participants is approximately 0.0054 (about 0.54%). In other words, such a large improvement would be very unlikely by chance alone if the programme were truly ineffective.

---

**(b) The researcher's conclusion is incorrect.**

The p-value is a conditional probability — it is $P(\text{observed result or more extreme} \mid H_0 \text{ is true})$, **not** the probability that $H_0$ is true. The statement *"there is a 0.54% probability that the programme has no effect"* incorrectly treats the p-value as the probability of the null hypothesis itself. This is a common misinterpretation; the p-value says nothing directly about the probability that $H_0$ is true or false.

---

**(c)** Since p-value $= 0.0054 < \alpha = 0.05$, we **reject $H_0$**. There is sufficient evidence at the 5% significance level to conclude that the 8-week cognitive training programme produces a statistically significant improvement in working memory scores (estimated mean improvement: 4.3 points).

---

**(d)** The hypothesis test finds a **statistically significant** result ($p = 0.0054$), meaning the improvement of 4.3 points is unlikely to be due to random chance. However, the improvement is **not practically significant** because 4.3 points is below the minimum clinically important difference of 10 points — it would not meaningfully improve a patient's daily functioning.

This illustrates that with a sufficient sample size, even very small effects can be detected as statistically significant. A complete evaluation of an intervention must consider both the statistical test (is the effect real?) and the **effect size** (is the effect large enough to matter?).
</div>
</details>
...
A social media company tests whether a small redesign of its "Like" button increases user engagement. In a randomised A/B test, $n_1 = 50{,}000$ users see the original button and $n_2 = 50{,}000$ see the redesigned button. The engagement rates are:

- **Original:** 18 650 users clicked the button ($\hat{p}_1 = 0.3730$)
- **Redesigned:** 18 800 users clicked the button ($\hat{p}_2 = 0.3760$)

A two-proportion $z$-test yields $z^* \approx 2.19$ and p-value $\approx 0.029$.

(a) At $\alpha = 0.05$, what is the conclusion of the hypothesis test?

(b) The absolute difference in engagement rates is $\hat{p}_2 - \hat{p}_1 = 0.376 - 0.373 = 0.003$, or 0.3 percentage points. The product manager argues that a 0.3-percentage-point increase is too small to justify the cost of redesigning the button. Explain whether you agree with the manager's concern, in the context of **practical significance**.

(c) Explain why this study found a statistically significant result even though the observed difference was so small.

(d) Describe **one** other piece of information, beyond the p-value, that would help the company decide whether to adopt the new button design.
</div>

<details data-type="hint">
<summary>💡 Hint</summary>
<div class="content">
- A small p-value only tells us that the effect is **unlikely to be zero** — it does not tell us that the effect is **large enough to matter**.
- Very large sample sizes can make trivially small differences statistically significant.
- Effect size measures (e.g., difference in proportions, relative risk, number needed to treat) help assess practical importance alongside p-values.
- Confidence intervals quantify the uncertainty around the estimated effect size.
</div>
</details>
...
<summary>✅ Answer</summary>
<div class="content">
**(a)** Since p-value $\approx 0.029 < \alpha = 0.05$, we **reject $H_0$**. There is sufficient evidence at the 5% significance level that the redesigned button produces a higher engagement rate than the original button.

---

**(b)** The product manager's concern is well-founded from a **practical significance** standpoint. An absolute difference of only 0.3 percentage points is extremely small in business terms. If implementing the redesign costs \$50{,}000, for example, the additional engagement may not generate enough revenue to justify the expense. Statistical significance does not guarantee that an effect is large enough to be worth acting on.

---

**(c)** The study found a statistically significant result primarily because of the **enormous sample size** ($n = 100{,}000$ total users). With such a large $n$, the standard error of the difference $\hat{p}_2 - \hat{p}_1$ is very small (approximately $0.00137$), so even a tiny true difference of 0.3 percentage points is distinguishable from zero with high probability. In general, larger samples give more power to detect small effects.

---

**(d)** One useful additional piece of information is a **confidence interval** for the true difference $p_2 - p_1$. For example, a 95% CI might be approximately $(0.0003,\ 0.0057)$, meaning the true effect is likely between 0.03 and 0.57 percentage points. This helps the company assess whether the **plausible range** of the effect size justifies the investment, beyond just knowing the direction of the effect.
</div>
</details>
...
$$\text{p-value} = P(F_{14,11} > 4.198) \approx 0.0109$$
<a href="foundations-of-inference.html#distribution-calculator" target="_blank" style="font-size:0.85em; color:#2c7bb6;">�� Distribution Calculator</a>

The critical value $F_{0.05}(14,11) \approx 2.739$. Since $F^* = 4.198 > 2.739$, we confirm rejection.

**(d)** Since p-value $\approx 0.011 < \alpha = 0.05$, we **reject $H_0$**. There is sufficient evidence to conclude that Laboratory 1 produces significantly more variable blood-glucose readings than Laboratory 2.
</div>
</details>
...
Since $F_{0.01}(12,9) \approx 4.39$ and $6.676 > 4.39$, the exact p-value $\approx 0.0039 < 0.01$.
...
A student attempts a hypothesis test for each scenario below, but makes **one error** in each attempt. Identify the error and explain how to correct it.

**(a)** A researcher tests $H_0: \mu = 50$ vs. $H_a: \mu > 50$ using a one-sample $t$-test with $n = 20$ observations. He computes $t^* = 2.14$ and reports:

$$\text{p-value} = P(T_{19} < 2.14) \approx 0.977$$

**(b)** A student tests $H_0: \mu_d = 0$ vs. $H_a: \mu_d \ne 0$ on $n = 8$ pairs using a paired $t$-test. She computes $t^* = -2.45$, uses $df = 16$ (total observations minus 2), and reports: $\text{p-value} = 2 \times P(T_{16} > 2.45) \approx 0.026$.

**(c)** An analyst tests $H_0: p = 0.30$ vs. $H_a: p \ne 0.30$ using a one-sample $z$-test for a proportion. With $n = 100$ and $x = 35$ successes, she computes:

$$z^* = \frac{0.35 - 0.30}{\sqrt{0.35 \times 0.65 / 100}} \approx 1.054$$

**(d)** A student tests $H_0: \sigma^2 = 25$ vs. $H_a: \sigma^2 < 25$ with $n = 12$ and $s = 3.8$. She computes $\chi^{2*} = 11(3.8^2)/25 = 6.33$, looks up the right-tail probability, and reports: $\text{p-value} = P(\chi^2_{11} > 6.33) \approx 0.850$.
</div>

<details data-type="hint">
<summary>💡 Hint</summary>
<div class="content">
- (a) Check the direction of the tail used for the p-value. For $H_a: \mu > \mu_0$, the p-value is the **upper** tail.
- (b) Check the degrees of freedom for a paired $t$-test. The paired test works on the **differences**, not the original observations.
- (c) Check which proportion is used in the standard error formula under $H_0$.
- (d) Check which tail is appropriate for $H_a: \sigma^2 < \sigma_0^2$.
</div>
</details>
...
<summary>✅ Answer</summary>
<div class="content">
**(a) Error: wrong tail used for the p-value.**

For $H_a: \mu > 50$ (upper-tailed test), the p-value is the **upper** tail probability: $P(T_{19} > 2.14)$, not the lower tail. The researcher computed $P(T_{19} < 2.14) \approx 0.977$, which is the probability of **not** rejecting $H_0$ under $H_a$, not the p-value.

**Correction:** $\text{p-value} = P(T_{19} > 2.14) = 1 - 0.977 \approx 0.023$. Since $0.023 < 0.05$, we reject $H_0$.

---

**(b) Error: wrong degrees of freedom for the paired $t$-test.**

In a paired $t$-test, we work with the $n = 8$ **differences** (one per pair), not with the $2n = 16$ original observations. The degrees of freedom are $df = n - 1 = 7$, not $2n - 2 = 14$.

**Correction:** $\text{p-value} = 2 \times P(T_{7} > 2.45) \approx 2(0.022) = 0.044$. Using $df = 7$ slightly increases the p-value compared to $df = 16$, but the conclusion at $\alpha = 0.05$ remains: reject $H_0$.

---

**(c) Error: wrong proportion used in the standard error under $H_0$.**

For the one-sample $z$-test for a proportion, the standard error in the denominator must be computed using the **null proportion $p_0$**, not the sample proportion $\hat{p}$. Using $\hat{p}$ in the SE conflates estimation with hypothesis testing.

**Correction:**
$$z^* = \frac{\hat{p} - p_0}{\sqrt{p_0(1-p_0)/n}} = \frac{0.35 - 0.30}{\sqrt{0.30 \times 0.70 / 100}} = \frac{0.05}{0.04583} \approx 1.091$$

The p-value for the two-sided test is $2 \times P(Z > 1.091) \approx 0.275$.

---

**(d) Error: used the right-tail probability for a left-tailed alternative.**

For $H_a: \sigma^2 < 25$ (lower-tailed test), a small $\chi^{2*}$ provides evidence against $H_0$. The p-value should be the **lower** tail probability $P(\chi^2_{11} < 6.33)$, not the upper tail.

**Correction:** $\text{p-value} = P(\chi^2_{11} < 6.33) \approx 0.150$. Since $0.150 > 0.05$, we fail to reject $H_0$. Note that the upper-tail value of approximately 0.850 is not the p-value — it is $1 - \text{p-value}$.
</div>
</details>
...
**Part 1 �ü F-test for equal variances**
...
### Graphical and Data-Based Exercises

The following exercises each include a plot generated from sample data. Read the figure carefully before answering the questions.
...
```{r ch6-q35-hist, echo=FALSE, fig.height=3.5, fig.width=6, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
ggplot(df_sleep, aes(x = hours)) +
  ...
  labs(x = "Nightly sleep duration (hours)", y = "Frequency",
       caption = "n = 16 volunteers  |  Dashed line = 渭�ü = 7.0 hours") +
...
```{r ch6-q36-bar, echo=FALSE, fig.height=3.5, fig.width=4.5, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
df_pets <- data.frame(
...
ggplot(df_pets, aes(x = outcome, y = count, fill = outcome)) +
  ...
  labs(x = NULL, y = "Number of households",
       caption = "n = 150 households  |  Dashed line = national benchmark (p�ü � n = 90)") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "none",
...
```{r ch6-q37-hist, echo=FALSE, fig.height=3.5, fig.width=6, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
ggplot(df_study, aes(x = hours)) +
  ...
  annotate("text", x = 8.5, y = 6.4, label = "卤��ü = 卤4 from mean",
           color = "#e17055", size = 3.5, hjust = 0) +
  scale_x_continuous(breaks = seq(6, 36, 3)) +
  labs(x = "Weekly study hours", y = "Frequency",
       caption = "n = 25 engineering students  |  Dashed lines = mean 卤 ��ü") +
  theme_minimal(base_size = 12) +
  theme(plot.caption = element_text(color = "#555", size = 9))
```
...
```{r ch6-q38-bar, echo=FALSE, fig.height=3.5, fig.width=6.5, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r ch6-q39-box, echo=FALSE, fig.height=3.5, fig.width=6, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
- `var.test()` automatically places the larger variance in the numerator. The reported F-statistic equals $s_A^2/s_B^2$.
...
$$\text{p-value} = 2 \times P(F_{19,17} > 1.675) \approx 0.380$$
...
Seven university students complete a self-reported anxiety questionnaire (scored 0�ü50; higher = more anxious) before and after a six-week mindfulness course. The plot below shows each student's anxiety score before and after the program.

```{r ch6-q40-line, echo=FALSE, fig.height=3.6, fig.width=5.5, fig.align='center', message=FALSE, warning=FALSE}
library(ggplot2)
...
ggplot(df_anxiety, aes(x = time, y = score, group = student)) +
  ...
  labs(x = NULL, y = "Anxiety score (0�ü50)", color = NULL,
